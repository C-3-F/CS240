        ...
        for (int i = guessCount; i > 0; i--) {
            ...
            try {
                ...
                for (int j = 0; j < currKeyChars.length; j++) {
                    if (currKeyChars[j] == guessChar) {
                        newLetterCount += 1;
                    }
                }

                if (newLetterCount > 1) {
                    System.out.println(String.format("Yes, there are %d %c's\n", newLetterCount, guessChar));
                    System.out.println("Size: " + returnedWords.size());
                    i += 1;
                } else if (newLetterCount > 0) {
                    System.out.println(String.format("Yes, there is %d %c\n", newLetterCount, guessChar));
                    i += 1;
                } else {
                    System.out.println(String.format("Sorry, there are no %c's\n", guessChar));
                }

                if (returnedWords.size() == 1 && currKey.equals(returnedWords.toArray()[0])) {
                ...
                System.out.println(String.format("You already guessed '%s'. Please try again\n", guessChar));
                i += 1;
                continue;
            }
